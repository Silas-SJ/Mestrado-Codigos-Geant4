...
//            if(htime[j]->GetEntries()<1) continue;
 ...
 //          SignalDigi->SetOverThresholdTimeVector(timevector);
        ...
        /*
      for(unsigned i = 0;i<timevec0.size();i++) G4cout << "timevec0[" << i << "] = " << timevec0[i] << G4endl;
      for(unsigned i = 0;i<timevec1.size();i++) G4cout << "timevec1[" << i << "] = " << timevec1[i] << G4endl;
      for(unsigned i = 0;i<pmtvec.size();i++) {
       G4cout << "pmt time[" << pmtvec[i] << "] = " << timevec[i] << G4endl;
      }
      for(auto j=timevector.begin();j!=timevector.end();++j) {for(unsigned i = 0;i<timevector[j->first].size();i++) G4cout << "timevector[" << j->first << "]" << "[" << i << "] = " << timevector[j->first][i] << G4endl;}       
