...
//    = (*fHitsCollection)[fHitsCollection->entries()-1];
  ...
  if(abs(step->GetTrack()->GetDefinition()->GetPDGEncoding()) == 211){
   ...
   {
   std::cout<<"Pion Capture at Rest:"<<step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName()<<std::endl;
   }
...
//   hit->SetPionTrackID(step->GetTrack()->GetTrackID());
   //G4cout << "Pion TrackID: " << step->GetTrack()->GetTrackID() << "  detNo: " << detNo << "  layer: " << layerNumber<< "  Collection name: " << collectionName[0]  << G4endl;
   /*
  if (eventID == 0 || eventID == 1 || eventID == 2 || eventID == 3 || eventID == 4 || eventID == 5 || eventID == 6 || eventID == 7 || eventID == 8 || eventID == 9)
 // {
    ...
    if (step->GetPreStepPoint()->GetProcessDefinedStep() && step->GetPreStepPoint()->GetProcessDefinedStep()->GetProcessName())
  {
   ...
   if (step->GetPreStepPoint()->GetPhysicalVolume() && step->GetPreStepPoint()->GetPhysicalVolume()->GetName())
    ...
    std::cout<< "Number layer:" <<layerNumber<< std::endl;
     }
  }
...
//   hit->SetMuonTrackID(step->GetTrack()->GetTrackID());
  // G4cout << "Muon TrackID: " << step->GetTrack()->GetTrackID() << "  detNo: " << detNo << "  layer: " << layerNumber<< "  Collection name: " << collectionName[0] << G4endl;
  }
  if ((step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName()=="Scintillation"||step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName()=="Decay")&& abs(step->GetTrack()->GetDefinition()->GetPDGEncoding()) == 211){ // Decay -> Scintillation ????? 
      piondecay=true;
    //  std::cout << "pion decay at layer " << step->GetPreStepPoint()->GetTouchableHandle()->GetCopyNumber(1) << "  Collection name: " << collectionName[0] << std::endl;
    }
  if ((step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName()=="Scintillation"||step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName()=="Decay") && abs(step->GetTrack()->GetDefinition()->GetPDGEncoding()) == 13){
      muondecay=true;
    //  std::cout << "muon decay at layer " << layerNumber << "  Collection name: " << collectionName[0] << std::endl;
    }
  // Add values
 //if ( edep==0. && stepLength == 0.) return false;
